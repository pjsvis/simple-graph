...
class SafeTestRunner {
  ...
  async runSafeTests() {
    ...
    try {
      ...
      try {
        ...
        console.error(`Additional error during verification: ${verifyError.message}`)
      }
  ...
  parseTestResults(stdout, stderr) {
    ...
    const errorLines = stderr.split('\n').filter(line => 
      line.includes('Error:') || line.includes('FAIL') || line.includes('SQLITE_BUSY')
    )
  ...
  static showUsage() {
    ...
    console.log('')
    console.log('Usage: node scripts/run-safe-tests.cjs [test-command] [options]')
    console.log('')
