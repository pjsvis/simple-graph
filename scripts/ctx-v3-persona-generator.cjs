...
class CtxV3PersonaGenerator {
  constructor() {
    this.dbPath = 'data/databases/the-loom-v2.db'
    this.db = null
  }
  ...
  async extractPHIDirectives() {
    ...
    const sql = `
      ...
      ORDER BY CAST(SUBSTR(json_extract(body, '$.directive_id'), 5) AS INTEGER)
    `
  ...
  async extractTopOPMDirectives() {
    ...
    const sql = `
      SELECT 
        n.id,
        json_extract(n.body, '$.id') as node_id,
        ...
        (
          SELECT COUNT(*) FROM edges e1 WHERE e1.source = json_extract(n.body, '$.id')
        ) + (
          SELECT COUNT(*) FROM edges e2 WHERE e2.target = json_extract(n.body, '$.id')
        ) as total_connections
      FROM nodes n
      WHERE json_extract(n.body, '$.id') LIKE 'cda-61-opm-%'
  ...
  async generatePersona() {
    console.log('\n🎨 GENERATING CTX-V3-BASELINE PERSONA')
...
`
      ...
      // Add PHI directives in numerical order
...
`
      ...
      // Add COG directives in numerical order
...
`
      ...
      // Add top OPM directives ordered by connectivity
...
`
      ...
      // Write the file
      const outputPath = 'ctx-v3-baseline.md'
...
if (require.main === module) {
  const generator = new CtxV3PersonaGenerator()
  generator.generatePersona().catch(console.error)
}

module.exports = { CtxV3PersonaGenerator }
