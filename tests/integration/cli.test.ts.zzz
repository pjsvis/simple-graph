...
import { SimpleGraph } from '../../src/SimpleGraph'; // Corrected import
...
describe('CLI Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      await execPromise(`bun build ${CLI_PATH} --outfile ${compiledCliPath} --target=node --format=esm --external=sqlite3`);
    } catch (error) {
